...
import type { Hotel, Team, HotelDocument, DocumentCategory, PSPIntegration, PMSIntegration, AutomationSettings, HotelUser } from '../types';
...
const IntegrationsTab: React.FC<{ formData: Hotel; setFormData: React.Dispatch<React.SetStateAction<Hotel>> }> = ({ formData, setFormData }) => {
    ...
    const handlePmsChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        setFormData(prev => ({...prev, integrations: {...prev.integrations, pms: {...prev.integrations.pms, type}}}));
    };

    const togglePspConnection = () => {
        const currentStatus = formData.integrations.psp.status;
        const newStatus = currentStatus === 'connected' ? 'not_connected' : 'connected';
        setFormData(prev => ({ ...prev, integrations: { ...prev.integrations, psp: {...prev.integrations.psp, status: newStatus }}}));
    };
    
    const testPmsConnection = () => {
        // Mock success/failure
        const newStatus = Math.random() > 0.3 ? 'connected' : 'error';
        setFormData(prev => ({ ...prev, integrations: { ...prev.integrations, pms: {...prev.integrations.pms, status: newStatus }}}));
    };
        ...
        <div className="space-y-6">
            <div className="p-4 border border-slate-800 rounded-lg">
                ...
                <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4 items-center">
                    ...
                    <div className="flex items-center space-x-4 mt-2">
                        ...
                        <StatusPill status={formData.integrations.psp.status} />
                         <button onClick={togglePspConnection} className="text-sm font-medium text-cyan-500 hover:underline">
                            {formData.integrations.psp.status === 'connected' ? 'Disconnect' : 'Connect'}
                        </button>
                    </div>
                </div>
            </div>
             <div className="p-4 border border-slate-800 rounded-lg">
                ...
                <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4 items-center">
                    <div>
                        <label htmlFor="pms-type" className="block text-sm font-medium text-slate-400">System</label>
                        <select id="pms-type" value={formData.integrations.pms.type} onChange={handlePmsChange} className={`mt-1 ${darkSelectStyle} capitalize`}>
                            <option value="none">None</option>
                     ...
                     <div className="flex items-center space-x-4 mt-2">
                        <span className="text-sm font-medium text-slate-400">Status:</span>
                        <StatusPill status={formData.integrations.pms.status} />
                         <button onClick={testPmsConnection} className="text-sm font-medium text-cyan-500 hover:underline">
                            Test Connection
                        </button>
                    </div>
                </div>
...
export const HotelEditModal: React.FC<HotelEditModalProps> = ({ hotel, onSave, onClose }) => {
    ...
    const handleSave = () => {
        ...
        if (Object.keys(errors).length === 0) {
            ...
            setActiveTab('details'); // Switch to tab with error
        }
    };
    
    const isNewHotel = !hotel.name; // Simple check to see if this is a new hotel being added
            ...
            <div className="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div className="inline-block align-bottom bg-slate-900 rounded-xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full">
                    ...
                    <div className="px-6 py-6 max-h-[60vh] overflow-y-auto">
                        ...
                        {activeTab === 'integrations' && <IntegrationsTab formData={formData} setFormData={setFormData} />}
