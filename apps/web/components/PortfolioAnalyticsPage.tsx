...
export const PortfolioAnalyticsPage: React.FC = () => {
    ...
    const analytics = useMemo(() => {
        ...
        const byProperty = disputes.reduce((acc, dispute) => {
            if (!acc[dispute.organizationId]) {
                acc[dispute.organizationId] = {
                    // In a real app, we would fetch hotel names. Here we use IDs.
                    propertyName: `Property ${dispute.organizationId.split('_')[1]}`,
                    disputeCount: 0,
                    wonCount: 0,
                    concludedCount: 0,
                    recoveredAmount: 0,
            ...
            const prop = acc[dispute.organizationId];
            prop.disputeCount++;
