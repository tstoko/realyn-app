...
- **Test ai-core:** `cd packages/ai-core && npm test` (Jest; covers RAG prompt-injection helpers, evidence-planner ↔ RAG, argument-generator ↔ RAG).
...
- **Functions secrets:** `functions/.env` holds placeholder secrets for `defineSecret` values (STRIPE_SECRET_KEY, STRIPE_WEBHOOK_SECRET, OPENAI_API_KEY, ANTHROPIC_API_KEY). Replace with real test keys if needed.
- **LLM provider:** The LLM service is provider-agnostic behind `packages/ai-core/src/services/llm/`. Pick the active provider per Cloud Run service via `LLM_PROVIDER` (`openai` (default) or `anthropic`). Both `OPENAI_API_KEY` and `ANTHROPIC_API_KEY` are bound as secrets on `onEvidencePlanQueued`, `draftArgument`, and `seedDemoData`, so flipping providers is one env-var edit — no redeploy. Override the default model via `OPENAI_MODEL` / `ANTHROPIC_MODEL` when needed.
...
- Jest is configured in `functions/` (`npm test` from `functions/`, config in `functions/jest.config.js`, test files as `*.test.ts` under `functions/src`) and in `packages/ai-core` (`npm test` from `packages/ai-core`, config in `packages/ai-core/jest.config.js`). The dashboard uses Vitest (`npm test` from `packages/dashboard`); the website and `packages/shared` have no test runner configured.
...
- `packages/ai-core` is a workspace package (`packages/*` glob). `functions/` is NOT a workspace package — it has its own `node_modules` and links `@realyn/ai-core` via `file:../packages/ai-core`. `functions/tsconfig.json` maps `@realyn/ai-core` and `@realyn/ai-core/*` to `node_modules/@realyn/ai-core/dist` so TypeScript resolves package `exports` without moving the whole Functions project to `Node16` resolution (which would require `.js` extensions on relative imports).
