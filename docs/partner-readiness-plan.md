...
- ADRs: `docs/adr/0001-ontology.md` (✅ landed with P0.3); `docs/adr/0002` through `docs/adr/0009` to be written as their owning workstreams ship
...
- **P0.2 — deploy RAG to prod (= RAG C7):** ✅ **DONE 2026-05-17** (after PR #23 follow-up fix). End-state revisions: `draftargument-00018-quk` and `onevidenceplanqueued-00002-kub`. Both bind `ANTHROPIC_API_KEY` + `PINECONE_API_KEY`; `PINECONE_INDEX_NAME=realyn-rag-dev` set via `functions/.env.realyn-app`. End-to-end retrieval **verified working** — smoke tests on dice + zipworld both returned 5 chunks at top scores 9.45 / 6.86. Deploy went through GitHub Actions and required 5 follow-up PRs (#18 env file, #19 root `npm ci` in CI, #20 drop `storage` from deploy targets, #21 drop `firestore:rules`/`indexes`, **#23 the silent-vector-store-registration fix that's the real "RAG actually fires" PR**) and ~10 IAM grants on the deploy SA + service agents — full archaeology in `docs/rag-phase-1-handoff.md` §Actual deploy story.
  - **Rules + indexes deploy gap closed 2026-05-17 (PRs #24, #25):** Granted `roles/firebaserules.admin`, `roles/firebasestorage.admin`, `roles/datastore.indexAdmin` to the GitHub Actions SA; restored `firestore:rules` + `firestore:indexes` to `--only`. Deploy run `26006025208` shipped the 5 backlog commits of rules/indexes changes that had been merged to `main` but never deployed since 2026-03-26. **`storage` rules are still excluded** because Firebase Storage was never initialised on the `realyn-app` project (no default bucket) — one-time human action in the Firebase console to enable it, then add `,storage` back to `--only` (the IAM is already in place).
  - **Open follow-ups:** Anthropic credit is at $0 so the deployed planner falls back to deterministic templates for every LLM call — RAG context is retrieved but never consumed; topping up credit unblocks meaningful C8 delta.
- **P0.3 — `@realyn/ontology` skeleton:** ✅ **DONE 2026-05-17** (this PR). New workspace package at `packages/ontology/` ships the seven canonical types (`User`, `Dispute`, `EvidencePlan`, `EvidenceItem`, `DisputeArgument`, `Organization`, `Subscription`) with zod schemas, plus placeholder `Outcome` / `AuditEvent` for W2.x. `@realyn/shared/types.ts` and `@realyn/shared/billing.ts` are now back-compat re-export shims, so every existing `import { Dispute } from '@realyn/shared'` continues to work. Typechecks pass for ontology, ai-core, dashboard, functions; dashboard tests (11) all pass. **What's deferred to W1.1:** migrating `functions/` and `@realyn/ai-core` to consume `@realyn/ontology` directly (they still use their hand-rolled `src/types/*.ts` files); tightening schemas with `.strict()`; wiring `Outcome` / `AuditEvent` to real persistence. See `docs/adr/0001-ontology.md` for the full rollout plan.
