...
> **Status as of 2026-05-17:** ✅ **C7 is COMPLETE. RAG is LIVE in prod.** `draftargument-00018-quk` deployed at 2026-05-17T18:35:14Z with `PINECONE_INDEX_NAME=realyn-rag-dev`, `ANTHROPIC_API_KEY`, and `PINECONE_API_KEY` bound. New service `onevidenceplanqueued-00001-faj` deployed at 2026-05-17T18:47:46Z with Eventarc trigger in `eur3` (Firestore region) listening on `disputes/{disputeId}` updates. Both startup probes succeeded first try, no boot crashes. The `realyn-rag-dev` Pinecone index has 2284 vectors in the `rulebooks` namespace (Visa Public Rules + Mastercard Chargeback Guide Merchant Edition). **Open work:** C8 (post-RAG eval) is now unblocked — re-run `npm run rag:eval:baseline` once a few new disputes have flowed through the deployed planner and they'll come back with RAG-enriched outputs for delta comparison against `docs/eval/2026-05-rag-phase1-baseline.md`. The rigorous `--force-refresh` mode (~$5–6 per run) is still optional. Three prior bugs are now fixed: (1) the planner Firestore write-back rejecting `undefined` fields (`50167f2`), (2) the C3 script's all-zipworld diversity bug (resolved via round-robin org-bucket picker, `2611386`), and (3) the Strategy Advisor LLM being silently skipped on urgent disputes (`61b019a`).
>
> **The deploy itself was a multi-PR effort** — see [§Actual deploy story (2026-05-17)](#actual-deploy-story-2026-05-17) below. PR #16 (code-side secret binding) was just the start. PR #19, #20, #21 fixed three layers of CI that had been silently broken since 2026-04-24, and a series of GCP IAM grants on the deploy SA + service agents + project-level API enablements unblocked the actual Firebase deploy. Total elapsed: ~7 GitHub Actions runs, ~6 IAM role grants, 1 placeholder secret. The functions deploy SA `github-action-1093841209@realyn-app.iam.gserviceaccount.com` now has the full role set documented in `.github/workflows/deploy-functions.yml`.
...
| C7 — bind PINECONE_API_KEY on Cloud Functions, deploy | ✅ **Done 2026-05-17** | `draftargument-00018-quk` (2026-05-17T18:35) + new `onevidenceplanqueued-00001-faj` (2026-05-17T18:47). Both bind `ANTHROPIC_API_KEY` + `PINECONE_API_KEY` (from Secret Manager) and read `PINECONE_INDEX_NAME=realyn-rag-dev` from `functions/.env.realyn-app`. Eventarc trigger for Firestore-trigger function created in `eur3` (DB region). Took PR #16 (secret binding) + PR #18 (env file) + PR #19 (root `npm ci` in CI) + PR #20 (drop `storage` from deploy targets) + PR #21 (drop `firestore:rules`/`indexes`) + 6 IAM grants on the deploy SA + 3 service-agent bootstrap bindings + Cloud Billing API enablement + a `placeholder_disable_email` secret for `RESEND_API_KEY`. See [§Actual deploy story](#actual-deploy-story-2026-05-17). |
| C8 — re-run eval, compare to baseline | 🟢 **Unblocked**, awaiting new disputes flowing through deployed planner | After enough disputes have been processed by the new revision (probably need ≥3 to make the comparison meaningful), re-run `npm run rag:eval:baseline` in default `read-cached` mode and rename the output to `docs/eval/2026-05-rag-phase1-post-rag.md`. Write the Delta Summary by hand against `docs/eval/2026-05-rag-phase1-baseline.md`. |
