...
> **Status as of 2026-05-20:** ✅ C7 complete (since 2026-05-17). **Cost optimisation 2026-05-20:** hoisted scheme-rulebook retrieval out of the evidence-planner revision loop into the orchestrator — was being called once per attempt (`MAX_REVISION_ATTEMPTS=2` → up to 3 retrievals per dispute, observed in prod logs as 3× identical `chunksReturned=5 topScore=…` lines per `disputeId`). The query is deterministic per `(disputeId, stage, reasonCode)` so the duplicate calls were pure waste — Pinecone reads + rerank API spend. Now resolved once per dispute and threaded through `SpecialistContext.rulebookRagResult`. The planner falls back to live retrieval when the field is absent (preserves single-shot test paths). The `argumentGenerator` is unchanged — it has no revision loop, so it already retrieves once per dispute. Tests in `packages/ai-core/src/services/__tests__/evidencePlannerRag.test.ts` lock the contract.
>
