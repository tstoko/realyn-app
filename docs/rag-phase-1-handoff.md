...
> **Status as of 2026-05-08:** PR #11 in review, PR #12 in review, **PR #13 in review**. The `realyn-rag-dev` Pinecone index is **provisioned and populated** — 2284 vectors in the `rulebooks` namespace (Visa Public Rules + Mastercard Chargeback Guide Merchant Edition). **C3 baseline is captured (free, read-cached mode)** and lives at `docs/eval/2026-05-rag-phase1-baseline.md` — 3 disputes (1 zipworld + 1 dice + 1 real prod org `y3i1cZvgeu2KrSrY4VrA`), pulled from existing `evidencePlan` / `argumentDraft` Firestore documents that were generated by the deployed planner *before* `PINECONE_API_KEY` was bound on Functions, and are therefore pre-RAG by definition. Captured at `$0` cost. `claude-opus-4-6` outputs are real production-fidelity. **Awaiting human grading** of the rubric tables. The rigorous `--force-refresh` mode (~$5–6 per run) remains gated on the user adding Anthropic API credit, but is no longer the default and may be skipped entirely. Three prior bugs are now fixed: (1) the planner Firestore write-back rejecting `undefined` fields (`50167f2`), (2) the C3 script's all-zipworld diversity bug (resolved via round-robin org-bucket picker, `2611386`), and (3) **the Strategy Advisor LLM being silently skipped on urgent disputes** (the worst case to skip — fixed in `61b019a`, with regression test). Next unblocked code work is still **C7** (bind secret on deployed Functions + deploy) — currently gated on `FIREBASE_TOKEN` in Cursor secrets. **C8 (post-RAG comparison) becomes a free re-run of the same script** once C7 lands and a few new disputes have been processed by the deployed Functions.
