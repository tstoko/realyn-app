...
> **Status as of 2026-05-08:** PR #11 in review, PR #12 in review, **PR #13 in review**, **PR #14 (C7 code-side: bind `PINECONE_API_KEY` on deployed Functions) staged on `cursor/rag-phase-1-c7-bind-pinecone-secret`**. The `realyn-rag-dev` Pinecone index is **provisioned and populated** — 2284 vectors in the `rulebooks` namespace (Visa Public Rules + Mastercard Chargeback Guide Merchant Edition). **C3 baseline is captured (free, read-cached mode)** and lives at `docs/eval/2026-05-rag-phase1-baseline.md` — 3 disputes (1 zipworld + 1 dice + 1 real prod org `y3i1cZvgeu2KrSrY4VrA`), pulled from existing `evidencePlan` / `argumentDraft` Firestore documents that were generated by the deployed planner *before* `PINECONE_API_KEY` was bound on Functions, and are therefore pre-RAG by definition. Captured at `$0` cost. `claude-opus-4-6` outputs are real production-fidelity. **Awaiting human grading** of the rubric tables. The rigorous `--force-refresh` mode (~$5–6 per run) remains gated on the user adding Anthropic API credit, but is no longer the default and may be skipped entirely. Three prior bugs are now fixed: (1) the planner Firestore write-back rejecting `undefined` fields (`50167f2`), (2) the C3 script's all-zipworld diversity bug (resolved via round-robin org-bucket picker, `2611386`), and (3) **the Strategy Advisor LLM being silently skipped on urgent disputes** (the worst case to skip — fixed in `61b019a`, with regression test). The C7 **code change is now staged** (PR #14): `secrets: ["ANTHROPIC_API_KEY", "PINECONE_API_KEY"]` on `onEvidencePlanQueued` and `draftArgument` in `functions/src/handlers/aiDisputeHandlers.ts`. The remaining **deploy step** (`firebase functions:secrets:set` + `firebase deploy`) is still gated on `FIREBASE_TOKEN` / ADC in Cursor secrets. **C8 (post-RAG comparison) becomes a free re-run of the same script** once the deploy lands and a few new disputes have been processed by the deployed Functions.
...
| C7 — bind PINECONE_API_KEY on Cloud Functions, deploy | 🟡 **Code-side done (PR #14), deploy gated on FIREBASE_TOKEN** | `secrets: [...]` list updated on `onEvidencePlanQueued` + `draftArgument` (`cursor/rag-phase-1-c7-bind-pinecone-secret`). `firebase functions:secrets:set PINECONE_API_KEY` + `firebase deploy --only functions:onEvidencePlanQueued,functions:draftArgument` still needs to run from a workstation or a Cursor Cloud Agent with `FIREBASE_TOKEN` / ADC. Plus a Cloud Run `--update-env-vars=PINECONE_INDEX_NAME=…` per environment so prod points at the populated index. |
...
| `cursor/rag-phase-1-c7-bind-pinecone-secret` | PR #14 (to be opened) | C7 code-side: bind `PINECONE_API_KEY` on `onEvidencePlanQueued` + `draftArgument`, plus 2 stripe-test mock fixups for the pre-existing `admin.firestore().settings is not a function` failure introduced by `50167f2`. Deploy still gated on `FIREBASE_TOKEN`. | `cursor/rag-phase-1-provisioning-4164` |

Merge order: #11 → #12 → #13 → #14. Don't merge them in this session — the user wants to review.
...
**From the C7 secret binding (branch `cursor/rag-phase-1-c7-bind-pinecone-secret`, PR #14):**
...
**Deploy steps for C7 (still pending — needs `FIREBASE_TOKEN` / ADC):**

```bash
firebase functions:secrets:set PINECONE_API_KEY --project=realyn-app

gcloud run services update onevidenceplanqueued \
  --region=us-central1 --project=realyn-app \
  --update-env-vars=PINECONE_INDEX_NAME=realyn-rag-dev
gcloud run services update draftargument \
  --region=us-central1 --project=realyn-app \
  --update-env-vars=PINECONE_INDEX_NAME=realyn-rag-dev

firebase deploy --only functions:onEvidencePlanQueued,functions:draftArgument \
  --project=realyn-app
```
