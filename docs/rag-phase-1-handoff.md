...
> **Status as of 2026-05-25:** 🔁 **LLM provider switched from Anthropic to OpenAI.** The Anthropic credit-block (zero balance, every specialist LLM call falling back to deterministic templates) was unblocking C8 the slow way — by waiting on a billing change. PR (this branch) lifts the LLM call site behind an `LlmProvider` interface (`packages/ai-core/src/services/llm/`) with two concrete implementations (`AnthropicProvider`, `OpenAIProvider`) and an env-driven factory (`LLM_PROVIDER`, default `openai`). Both `OPENAI_API_KEY` and `ANTHROPIC_API_KEY` are now bound on the three secret-binding sites (`onEvidencePlanQueued`, `draftArgument`, `seedDemoData`), so flipping providers in prod is one env-var edit, no redeploy. The OpenAI path uses the native structured-output API (`chat.completions.parse` + `zodResponseFormat`), so JSON-conformance is enforced by the API rather than by prompt-engineered "respond ONLY with valid JSON" + parse-then-validate — a strict win for reliability over the Anthropic path it replaces by default. **Action required to unblock C8:** add `OPENAI_API_KEY` to GCP Secret Manager (`firebase functions:secrets:set OPENAI_API_KEY`), then redeploy. If the built-in default model (`gpt-5.5`) is not accepted by the OpenAI API on this account, override via the `OPENAI_MODEL` env var — no redeploy needed.
>
