...
const RELEVANCE_SCORER_SYSTEM_PROMPT = `You are an expert at evaluating evidence relevance for hotel chargeback disputes.
...
export async function scoreEvidenceRelevance(
  ...
  try {
    ...
    const result = await callLLM(prompt, EvidenceRelevanceScoresSchema, {
      systemPrompt: RELEVANCE_SCORER_SYSTEM_PROMPT,
      temperature: 0.2,
      maxTokens: 3000,
    });
...
function buildRelevanceScoringPrompt(
  ...
  if (existingEvidence && existingEvidence.availableDocuments.length > 0) {
    ...
    if (relevantDocs.length > 0) {
      ...
      parts.push("The hotel already has these relevant documents on file:");
  ...
  for (const evidenceType of HOTEL_EVIDENCE_TYPES) {
...
function generateFallbackScores(
  ...
  // Base scores by claim type
  const scoreMap: Record<string, number> = {};

  // Set base scores for all evidence types
  for (const evidenceType of HOTEL_EVIDENCE_TYPES) {
    scoreMap[evidenceType] = 30; // Default moderate relevance
  }

  // Adjust scores based on claim type
  switch (claimType) {
    case "fraud":
  ...
  // Build scores array
  const scores = HOTEL_EVIDENCE_TYPES.map((evidenceType) => {
