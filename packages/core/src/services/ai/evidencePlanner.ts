...
export async function generateEvidencePlan(
  ...
  try {
    ...
    const systemPrompt = resolveVerticalPrompt(vertical, !!context?.claimAnalysis);
...
const EVIDENCE_PLANNER_PROMPT_REGISTRY: Record<string, string> = {
  hospitality: EVIDENCE_PLANNER_SYSTEM_PROMPT,
  hospitality_enhanced: ENHANCED_EVIDENCE_PLANNER_SYSTEM_PROMPT,
  ticketing: TICKETING_EVIDENCE_PLANNER_SYSTEM_PROMPT,
  ticketing_enhanced: ENHANCED_TICKETING_EVIDENCE_PLANNER_SYSTEM_PROMPT,
};

function resolveVerticalPrompt(vertical: string, hasSpecialistContext: boolean): string {
  const vDef = verticalRegistry.resolve(vertical);
  const promptKey = hasSpecialistContext
    ? vDef.systemPrompts.enhancedEvidencePlanner
    : vDef.systemPrompts.evidencePlanner;
  return EVIDENCE_PLANNER_PROMPT_REGISTRY[promptKey] ?? EVIDENCE_PLANNER_SYSTEM_PROMPT;
}
...
function buildEvidencePlanPrompt(
  ...
  // Rich scheme rule context (KB-populated) takes priority over basic codeInfo
  const schemeRule = context?.schemeRule;
  if (schemeRule && (schemeRule.merchantObligation || schemeRule.citations.length > 0)) {
    parts.push("## Card Network Scheme Rules");
    parts.push(`- **Code**: ${schemeRule.code} (${schemeRule.network.toUpperCase()}) — ${schemeRule.description}`);
    parts.push(`- **Category**: ${schemeRule.category}${schemeRule.subcategory ? ` / ${schemeRule.subcategory}` : ""}`);
    if (schemeRule.merchantObligation) {
      parts.push(`- **Merchant obligation**: ${schemeRule.merchantObligation}`);
    }
    if (schemeRule.cardholderBurden) {
      parts.push(`- **Cardholder burden**: ${schemeRule.cardholderBurden}`);
    }
    parts.push(`- **Required Evidence Categories**: ${schemeRule.requiredEvidence.join(", ")}`);
    if (schemeRule.optionalEvidence.length > 0) {
      parts.push(`- **Optional Evidence Categories**: ${schemeRule.optionalEvidence.join(", ")}`);
    }
    if (schemeRule.submissionConstraints.length > 0) {
      parts.push(`- **Submission constraints**: ${schemeRule.submissionConstraints.join("; ")}`);
    }
    if (schemeRule.timeLimit.days > 0) {
      parts.push(`- **Time limit**: ${schemeRule.timeLimit.days} days from ${schemeRule.timeLimit.fromEvent}`);
    }
    if (schemeRule.citations.length > 0) {
      parts.push("**Relevant citations:**");
      for (const cite of schemeRule.citations) {
        parts.push(`  - ${cite.section}: "${cite.excerpt}"`);
      }
    }
    parts.push(`- **Default Recommendation**: ${schemeRule.defaultRecommendation}`);
    parts.push("");
  } else if (codeInfo) {
    ...
    if (codeInfo.optionalEvidence.length > 0) {
      parts.push(`- **Optional Evidence Categories**: ${codeInfo.optionalEvidence.join(", ")}`);
    }
    parts.push("");
  }

  // Vertical-specific evidence requirements from KB
  const evReqs = context?.evidenceRequirements;
  if (evReqs && evReqs.requirements.length > 0) {
    parts.push("## Vertical-Specific Evidence Requirements");
    parts.push(`For reason code ${evReqs.reasonCode} in the ${evReqs.verticalId} vertical:\n`);
    for (const req of evReqs.requirements) {
      parts.push(`- **${req.evidenceType}** (${req.category}, priority: ${req.priority})`);
      parts.push(`  Rationale: ${req.rationale}`);
      if (req.tips.length > 0) {
        parts.push(`  Tips: ${req.tips.join("; ")}`);
      }
      if (req.canAutoFulfill) {
        parts.push(`  Auto-fulfillable from: ${req.sourceSystem || "PMS"}`);
      }
    }
    parts.push("");
  }
  ...
  // PSP format guidance
  const pspFormats = context?.pspFormats;
  if (pspFormats && pspFormats.length > 0) {
    parts.push("## PSP Evidence Format Requirements");
    parts.push("The PSP has specific format requirements for evidence slots:\n");
    for (const fmt of pspFormats) {
      const formats = fmt.acceptedFormats.join(", ");
      const size = fmt.maxSizeBytes ? ` (max ${(fmt.maxSizeBytes / 1024 / 1024).toFixed(1)}MB)` : "";
      parts.push(`- **${fmt.evidenceSlot}** (${fmt.apiFieldName}): accepts ${formats}${size}${fmt.isRequired ? " — REQUIRED" : ""}`);
    }
    parts.push("When planning evidence, note the required format for each item so the user provides the right type.");
    parts.push("");
  }

  // Win patterns from KB
  const winPatterns = context?.winPatterns;
