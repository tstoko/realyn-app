...
interface CommandPaletteProps {
  onClose: () => void;
  onNavigate: (view: 'properties' | 'portfolio_analytics' | 'activity_log' | 'user_management') => void;
  isAdmin: boolean;
}
...
export const CommandPalette: React.FC<CommandPaletteProps> = ({ onClose, onNavigate, isAdmin }) => {
  ...
  const actions: Action[] = useMemo(() => [
    ...
    { id: 'nav-user-management', title: 'Go to User Management', section: 'Navigation', action: () => onNavigate('user_management'), adminOnly: true },
  ], [onNavigate]);
    ...
    <div className="fixed z-50 inset-0 overflow-y-auto" role="dialog" aria-modal="true">
      <div className="flex items-start justify-center min-h-screen pt-20 px-4 text-center">
        ...
        <div className="relative inline-block align-bottom bg-slate-900 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-xl sm:w-full border border-slate-700">
          ...
          <div className="border-t border-slate-800 px-4 py-2.5 flex items-center justify-between text-xs text-slate-500">
            <span>
              <kbd className="px-1.5 py-0.5 bg-slate-800 border border-slate-700 rounded text-slate-400 mr-1">↑↓</kbd>
              to navigate
              <kbd className="px-1.5 py-0.5 bg-slate-800 border border-slate-700 rounded text-slate-400 mx-1">↵</kbd>
              to select
            </span>
            <span>
              Press <kbd className="px-1.5 py-0.5 bg-slate-800 border border-slate-700 rounded text-slate-400">?</kbd> for all shortcuts
            </span>
          </div>
        </div>
      </div>
