...
export const StatCard: React.FC<StatCardProps> = ({ 
  ...
  const baseClasses = "group relative bg-slate-900/50 backdrop-blur-sm rounded-2xl p-5 border border-slate-800 hover:border-slate-700 transition-all duration-300 hover:shadow-lg hover:shadow-cyan-900/10 overflow-hidden";
    ...
    <div className={`${baseClasses} ${className}`}>
      ...
      <div className="relative flex items-start justify-between">
        <div className="flex-1 min-w-0">
          ...
          <p className={`text-2xl sm:text-3xl font-bold ${valueClassName} tracking-tight min-w-0 flex-1 truncate font-heading`}>
             {value}
          </p>
          {description && (
