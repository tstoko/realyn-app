...
import type { OrgDocumentLoader, OrgDocument } from "../../ports";

type HotelDocument = OrgDocument;
 ...
 * @param orgDocLoader - Port implementation that loads org documents
 * @param disputeCase - Optional dispute case for LLM-powered analysis
 * @param claimAnalysis - Optional claim analysis for argument coverage assessment
 */
export async function analyzeExistingEvidence(
  organizationId: string,
  orgDocLoader: OrgDocumentLoader,
  disputeCase?: DisputeCase,
  claimAnalysis?: ClaimAnalysis
): Promise<ExistingEvidenceAnalysis | null> {
  try {
    const documents: HotelDocument[] = await orgDocLoader.getOrgDocuments(organizationId);
